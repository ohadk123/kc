...
#define CHECKER_ERROR(c, l, fmt, ...) c->hadError = compile_err_no_abort((c)->unit->fileName, l, fmt, ##__VA_ARGS__)
...
static Stmt *expect_var(Checker *c, String name, Location loc) {
    ...
    if (!found)
        CHECKER_ERROR(c, loc, "Unkown symbol '%.*s'", STRF(name));
...
static Stmt *expect_func(Checker *c, String name, Location loc) {
    ...
    if (!found)
        CHECKER_ERROR(c, loc, "Call to undeclared function '%.*s'", STRF(name));
    else if (found->kind != STMT_FUNC)
        CHECKER_ERROR(c, loc, "Symbol cannot be called");
    return found;
}
...
static Type *check_func_call(Checker *c, Expr *e) {
    ...
    if (argsCount != paramCount) {
        if (argsCount > paramCount)
            CHECKER_ERROR(c, e->loc, "Too many arguments to function call expected %zu, got %zu", paramCount,
                          argsCount);
...
static void check_switch(Checker *c, Stmt *s) {
    ...
    for (size_t i = 0; i < s->as.switchS.cases.len; i++) {
        ...
        if (!eval_expr(case_->value, &case_->eval))
            CHECKER_ERROR(c, case_->value->loc, "Switch case value is not compile time constant");
